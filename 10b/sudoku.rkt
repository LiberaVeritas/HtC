...
;; a solution to a given board, false if no solution
;; assume given board is valid


;; given board with one spot left
;; find next empty square
;; generate all possible 1-9 for the square
;; check for validity against its peers
;; remove non valid
;; check for solved

;; given unsolved
;; find next empty
;; generate all possible in a list
;; remove non valid
;; for each in list
;; recurse
;; base is if next empty 81
...
(define TEST (fill-square BD4s 80 B))
(define TEST2 (fill-square BD4s 0 B))

(check-expect (solve TEST) BD4s)
...
(define (solve b)
  (local [(define (next-blank b)
            (cond [(empty? b) 0]
                  [(false? (first b)) 0]
                  [else
                   (add1 (next-blank (rest b)))]))

          (define (valid?-p p)
            (λ (b)
              (local [(define (read-square-b p)
                        (read-square b p))
                  
                      (define val (read-square-b p))

                      (define (peers p lol)
                        (remove p
                                (if (member p (first lol))
                                    (first lol)
                                    (peers p (rest lol)))))
                  
                      (define row
                        (map read-square-b (peers p ROWS)))
                      (define col
                        (map read-square-b (peers p COLS)))
                      (define box
                        (map read-square-b (peers p BOXES)))]
            
                (not (or (member val row)
                         (member val col)
                         (member val box))))))
          ...
          (define (gen-next b)
            (local [(define p (next-blank b))
                    (define (helper n)
                      (fill-square b p (add1 n)))]
              (filter (valid?-p p)
                      (build-list 9 helper))))]
(solve-b b)))

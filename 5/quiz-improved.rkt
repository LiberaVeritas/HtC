...
(define (sink lob)
  (cond [(empty? lob) empty]
        ...
        [else (drop (first lob) (sink (rest lob)))]))

;; Blob ListOfBlob -> ListOfBlob
;; drops the solid blob one position into the list

(check-expect (drop BS empty) (cons BS empty))
(check-expect (drop BS (cons BB (cons BS (cons BB empty))))
              (cons BB (cons BS (cons BS (cons BB empty)))))

(check-expect (drop BS (cons BS (cons BS (cons BB empty))))
              (cons BS (cons BS (cons BS (cons BB empty)))))


(define (drop s lob)
  (cond [(empty? lob) (cons s empty)]
        [(solid? (first lob)) (cons s lob)]
        [else (cons (first lob) (cons s (rest lob)))]))

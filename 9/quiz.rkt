...
;; arranges images according to the given function

(define (arrange-all fn loi)
  (foldr fn empty-image loi))
...
;; (Region (listof X) -> X) X Region -> X
;; fold for Region

(define (fold-region fn base r)
  (local [(define (fold-lor lor)
            (cond [(empty? lor) base]
                  [else
                   (cons (fold-region fn base (first lor))
                         (fold-lor (rest lor)))]))]
    (fn r (fold-lor (region-subregions r)))))

                   

;; Region -> (listof String)
;; list the names of all regions and subregions

(check-expect (all-regions CANADA)
              (list "Canada" "British Columbia" "Vancouver" "Victoria" "Alberta" "Calgary" "Edmonton"))

(define (all-regions r)
  (local [(define (fn r los)
            (cons (region-name r)
                  (foldr append empty los)))]
    (fold-region fn empty r)))
